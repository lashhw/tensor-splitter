...
class GroupInfo:
    """Resolved metadata for a contiguous linear-chain group."""
...
def analyze_group(nodes: List[gs.Node], indices: IndexPair) -> GroupInfo:
    """Validate and collect metadata for a linear chain of nodes."""
    start, end = indices
    if start < 0 or end >= len(nodes) or end < start:
        raise ValueError(f"invalid group indices {indices} for graph with {len(nodes)} nodes")

    group_nodes = nodes[start : end + 1]
